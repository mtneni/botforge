...
Astrid is a themed BotForge persona — a friendly, hippie Australian astrologer who believes the stars explain life.
...
The Astrid bot is a self-contained package in [`com.embabel.bot.astrid`](src/main/java/org/legendstack/bot/astrid/).
It is discovered at startup via the `botforge.bot-packages=com.embabel.bot.astrid

The package provides:

| Class | Purpose |
|---|---|
| [`AstridConfiguration`](src/main/java/org/legendstack/bot/astrid/AstridConfiguration.java) | Spring `@Configuration` — registers users, relations, RAG documents, and the daily horoscope subagent |
| [`AstrologyTools`](src/main/java/org/legendstack/bot/astrid/AstrologyTools.java) | `@LlmTool` methods the LLM can call: `calculateNatalChart`, `getSunSign`, `getHouseSystems` |
| [`DailyHoroscopeAgent`](src/main/java/org/legendstack/bot/astrid/DailyHoroscopeAgent.java) | Multi-step Embabel `@Agent` — geocodes birth place, calculates natal chart, gets current transits, interprets |
| [`GeocodingService`](src/main/java/org/legendstack/bot/astrid/GeocodingService.java) | Converts birth place names to latitude/longitude for chart calculation |
...
Astrid's [`Relations`](src/main/java/org/legendstack/bot/astrid/AstridConfiguration.java) bean defines the entity types and relationships
...
|---|---|---|
| `botforge-neo4j` | 7891 (Bolt), 8892 (HTTP) | Neo4j graph database for memory |
| `botforge-astrology-api` | 3000 | Swiss Ephemeris astrology API |
...
The [`DailyHoroscopeAgent`](src/main/java/org/legendstack/bot/astrid/DailyHoroscopeAgent.java) is a multi-step Embabel agent
...
The agent is registered as a `Subagent` tool in [`AstridConfiguration`](src/main/java/org/legendstack/bot/astrid/AstridConfiguration.java),
...
# 2. Run BotForge with the Astrid profile
SPRING_PROFILES_ACTIVE=astrid ./mvnw spring-boot:run
